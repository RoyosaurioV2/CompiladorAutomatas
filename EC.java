...
public class EC implements ECConstants {
    ...
    Expresion hacerBinaria(Token operador, Expresion izq, Expresion der, TipoDato tipoDato) {

        // Ensamblamiento (Folding)
        Expresion o = folding(izq, der, operador);

        if (o != null) return o;

        // Simplificación
        o = siplificacionArit(izq, der, operador);

        return new ExpresionBinaria(operador, izq, der, tipoDato);
    ...
    ExpresionConstante folding(Expresion izq, Expresion der, Token operador) {

        if (izq.getTipoExpresion() == Expresion.EXP_CONSANTE && der.getTipoExpresion() == Expresion.EXP_CONSANTE) {

            ExpresionConstante cIzq = (ExpresionConstante)izq;
            ExpresionConstante cDer = (ExpresionConstante)der;

            String valor = null;
            Object valorJava = null;
            TipoDato dato = cIzq.getTipo();

            if (cIzq.getTipo() == TipoDato.ENTERO) {

                Integer vIzq = (Integer)cIzq.getValorJava();
                Integer vDer = (Integer)cDer.getValorJava();
                Integer r = null;
                Boolean b = null;

                switch (operador.kind) {
            ...
            else if (cIzq.getTipo() == TipoDato.FLOTANTE) {

                Double vIzq = (Double)cIzq.getValorJava();
                Double vDer = (Double)cDer.getValorJava();
                Double r = null;
                Boolean b = null;
            ...
            else if (cIzq.getTipo() == TipoDato.CARACTER) {

                Character vIzq = (Character)cIzq.getValorJava();
                Character vDer = (Character)cDer.getValorJava();
                Character r = null;
                Boolean b = null;
            ...
            else if (cIzq.getTipo() == TipoDato.CADENA) {

                String vIzq = (String)cIzq.getValorJava();
                String vDer = (String)cDer.getValorJava();
                String r = null;
                Boolean b = null;
            ...
            else if (cIzq.getTipo() == TipoDato.BOOLEANO) {

                Boolean vIzq = (Boolean)cIzq.getValorJava();
                Boolean vDer = (Boolean)cDer.getValorJava();
                Boolean b = null;
            ...
            if (valor != null)
                return new ExpresionConstante(valor, valorJava, dato);
    ...
    Expresion siplificacionArit(Expresion izq, Expresion der, Token operador) {

        if (operador.kind != ECConstants.MULTIPLICACION) return null;

        ExpresionConstante cExp = null;
        Expresion exp = null;

        if (izq.getTipoExpresion() == Expresion.EXP_CONSANTE) {
            cExp = (ExpresionConstante)izq;
            exp = der;
        }

        if (der.getTipoExpresion() == Expresion.EXP_CONSANTE) {
            cExp = (ExpresionConstante) der;
            exp = izq;
        }
        ...
        if (cExp.getTipo() != TipoDato.ENTERO) return null;

        Integer valor = (Integer)cExp.getValorJava();
        Token suma = new Token(ECConstants.SUMA, "+");
    ...
    public class ExpresionConstante extends Expresion {
        ...
        private Object _valorJava;
        private TipoDato _tipo;

        public ExpresionConstante(String valor, Object valorJava, TipoDato tipo) {
            _valor = valor;
            _tipo = tipo;
            _valorJava = valorJava;
        }
  ...
  final public ExpresionConstante constantes() throws ParseException {
    ...
    case NUMERO_ENTERO:
      jj_consume_token(NUMERO_ENTERO);
        {if (true) return new ExpresionConstante(token.image, Integer.parseInt(token.image), TipoDato.ENTERO);}
      break;
    case NUMERO_DECIMAL:
      jj_consume_token(NUMERO_DECIMAL);
        {if (true) return new ExpresionConstante(token.image, Double.parseDouble(token.image), TipoDato.FLOTANTE);}
      break;
    case CADENA_TEXTO:
      jj_consume_token(CADENA_TEXTO);
        image = token.image;
        valor = image.substring(1, image.length() - 1);
        {if (true) return new ExpresionConstante(image, valor, TipoDato.CADENA);}
      break;
    case CARACTER_TEXTO:
      jj_consume_token(CARACTER_TEXTO);
        image = token.image;
        valor = image.substring(1, 2).charAt(0);

        {if (true) return new ExpresionConstante(image, valor, TipoDato.CARACTER);}
      break;
    case BOOLEANO_FALSO:
      jj_consume_token(BOOLEANO_FALSO);
        {if (true) return new ExpresionConstante(token.image, Boolean.FALSE, TipoDato.BOOLEANO);}
      break;
    case BOOLEANO_VERDADERO:
      jj_consume_token(BOOLEANO_VERDADERO);
        {if (true) return new ExpresionConstante(token.image, Boolean.TRUE, TipoDato.BOOLEANO);}
      break;
    default:
      jj_la1[11] = jj_gen;
      jj_consume_token(-1);
  ...
  final public Expresion comparacion(Ambito ambito) throws ParseException {
    ...
    default:
      ...
      ;
        ...
        {if (true) return hacerBinaria(op, izq, der, TipoDato.BOOLEANO).conError(error);}
    throw new Error("Missing return statement in function");
  }
  ...
  final public Expresion factor(Ambito ambito) throws ParseException {
    ...
    case LOGICO_NOT:
      ...
      exp = factor(ambito);
        ...
        } else if (exp.getTipoExpresion() == Expresion.EXP_CONSANTE) {

            ExpresionConstante cExp = (ExpresionConstante)exp;
            Boolean r = !((Boolean)cExp.getValorJava());

            {if (true) return new ExpresionConstante(r ? "Verdadero" : "Falso", r, TipoDato.BOOLEANO);}
        }
